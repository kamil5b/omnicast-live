...
(function () {
    ...
    let overlayRevealed = {}; // id -> roleText (per-player GM reveal to overlay)
    ...
    socket.on("gameState", (gs) => {
        overlayRevealed = gs.overlayRevealed || {};
        renderPortraits(gs);
        renderBuzzer(gs);
        renderVotes(gs);
    ...
    socket.on("buzzer:winner", ({ playerId, playerName }) => {
        ...
        buzzerBanner._timer = setTimeout(() => {
    ...
    socket.on("buzzer:reset", () => {
        buzzerBanner.style.display = "none";
        document
            .querySelectorAll(".portrait")
            .forEach((el) => el.classList.remove("winner"));
    });

    socket.on("rolesRevealed", (roleData) => {
        // Global show-all-roles: update overlayRevealed from the broadcast payload
        // (keys other than 'type' are playerID -> roleText).
        Object.entries(roleData).forEach(([id, role]) => {
            if (id === "type") return;
            overlayRevealed[id] = role;
    ...
    function createPortrait(p, size, gs) {
      ...
      <div class="portrait-role" id="role-${p.id}" style="max-width:${size}px;display:${overlayRevealed[p.id] ? "block" : "none"}">
        ${escHtml(overlayRevealed[p.id] || "")}
      </div>
    `;
        return el;
    }
    ...
    function updatePortrait(el, p, size, gs) {
        ...
        if (roleEl) {
            const roleText = overlayRevealed[p.id] || "";
    ...
    function renderVotes(gs) {
        ...
        voteResults.innerHTML = gs.revealedVotes
            .map(
                (r) => `
