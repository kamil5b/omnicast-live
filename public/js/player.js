...
(function () {
  ...
  try { savedProfile = JSON.parse(localStorage.getItem(STORAGE_KEY) || 'null'); } catch (_) {}
  ...
  socket.on('buzzer:enabled', () => {
    ...
    setTimeout(() => { buzzerStatus.textContent = ''; }, 2000);
  });
  ...
  function renderPlayerState(s) {
    ...
    if (lastPoints !== newPts) {
      ...
      ptsChange.style.color = diff > 0 ? 'var(--success)' : 'var(--danger)';
      setTimeout(() => { ptsChange.textContent = ''; }, 2000);
      lastPoints = newPts;
    }
  ...
  function renderVoteResults(results) {
    ...
    voteRevealList.innerHTML = results.map((r) => `
  ...
  // ── Voting UI ─────────────────────────────────────────────────────────────
  // Rebuild vote list from gameState on playerState update
  socket.on('gameState', (gs) => {
    if (!myState?.votingOpen) return;
    buildVoteList(gs.players);
  });

  // Also refresh vote list when playerState arrives
  function buildVoteList(players) {
    if (!players) return;
    voteList.innerHTML = '';
    Object.values(players).forEach((p) => {
      if (p.id === myId) return; // Can't vote for yourself
      const div = document.createElement('div');
      div.className = 'vote-target' + (p.id === selectedVoteTarget ? ' selected' : '');
      div.dataset.id = p.id;
      ...
      div.addEventListener('click', () => {
        ...
        document.querySelectorAll('.vote-target').forEach((el) => el.classList.remove('selected'));
  ...
  function startReveal(e) {
    e.preventDefault();
    holdTimer = setTimeout(() => { roleValue.classList.add('revealed'); }, 0);
  }
  function endReveal() { roleValue.classList.remove('revealed'); clearTimeout(holdTimer); }
