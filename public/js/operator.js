...
(function () {
  ...
  function renderAll(gs) {
    ...
    const parts = [];
    if (gs.buzzerLocked) parts.push(`Buzzer locked — winner: ${gs.players[gs.buzzerWinner]?.name || '?'}`);
  ...
  function renderScoreboard(players, modules) {
    if (!modules?.points) { scoreList.innerHTML = '<span style="color:var(--text-muted);font-size:.85rem">Points module disabled</span>'; return; }
  ...
  function renderPlayers(players) {
    ...
    playersGrid.addEventListener('click', async (e) => {
      ...
      if (action === 'url') {
        ...
        socket.emit('operator:overrideImage', { playerId: id, imageUrl: url });
      ...
      if (action === 'file') {
        ...
        try {
          ...
          if (data.filename) {
            socket.emit('operator:overrideImageUpload', { playerId: id, filename: data.filename });
            toast('Image updated', 'success');
          } else {
  ...
  // ── Global actions ────────────────────────────────────────────────────────
  document.getElementById('show-roles-btn').addEventListener('click', () => socket.emit('operator:showAllRoles', { show: true }));
  document.getElementById('hide-roles-btn').addEventListener('click', () => socket.emit('operator:showAllRoles', { show: false }));
  document.getElementById('reset-scores-btn').addEventListener('click', () => {
    if (confirm('Reset all scores to 0?')) socket.emit('operator:resetScores');
  });
